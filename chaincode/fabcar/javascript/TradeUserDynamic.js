...
class TradingPlatformContract extends Contract {
    async registerUser(ctx, userId, userDetails) {
        const userExists = await this.isUserExists(ctx, userId);
        if (userExists) {
            throw new Error('User already exists');
        }

        // Hash and salt the password before storing (if a password field is provided)
        let user = JSON.parse(userDetails);
        if (user.userPassword) {
            const salt = crypto.randomBytes(16).toString('hex');
            const hashedPassword = crypto.pbkdf2Sync(user.userPassword, salt, 1000, 64, 'sha512').toString('hex');
            user.userPassword = hashedPassword;
            user.passwordSalt = salt;
        }

        user.createdAt = new Date();
        user.createdBy = ctx.clientIdentity.getID();
